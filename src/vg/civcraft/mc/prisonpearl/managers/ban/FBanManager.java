...
public class FBanManager extends BanManager {
	...
	private void writeBanJournal() {
		...
		try {
			...
			PrisonPearlPlugin.log("Failed to append ban journal: " + e.toString());
			e.printStackTrace();
		} finally {
			try {
				if (fos != null) {
				...
				PrisonPearlPlugin.log("Failed to close ban journal: " + ie.toString());
			}
		}
	}
	...
	private void recordToBanJournal(FileOutputStream fos, UUID name, boolean isBanned) {
		...
		try {
			...
			PrisonPearlPlugin.log("Failed to append ban journal: " + ban_record);
			e.printStackTrace();
		}
	}
	...
	public Map<UUID, Boolean> readBanJournal(boolean openPreviousJournal) {
		...
		if (!activeJournal.exists()) {
			try {
				...
				PrisonPearlPlugin.log("Unable to create ban journal: " + activeJournal.getPath());
				e.printStackTrace();
			}
			return banState;
		}
		...
		try {
			...
			while (true) {
				...
				if (parts.length < 2) {
					PrisonPearlPlugin.log("Malformed line: " + line);
					continue;
				}
			...
			PrisonPearlPlugin.log("Failed to read ban journal");
			e.printStackTrace();
		} finally {
			try {
				if (br != null) {
	...
	private void rotateBanJournal() {
		try {
			...
			PrisonPearlPlugin.log("Failed to rotate ban journal");
			e.printStackTrace();
		}
	}
